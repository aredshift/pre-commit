...
def get_default_version() -> str:
    ...
    elif all(lang_base.exe_exists(exe) for exe in ('node', 'yarn')):
        return 'system'
    else:
        return C.DEFAULT
...
def install_environment(
    ...
    with in_env(prefix, version):
        ...
        local_install_cmd = (
            'yarn', 'install', '--include=dev', '--include=prod',
            '--ignore-prepublish', '--no-progress', '--no-save',
        )
        lang_base.setup_cmd(prefix, local_install_cmd)

        _, pkg, _ = cmd_output('yarn', 'pack', cwd=prefix.prefix_dir)
        pkg = prefix.path(pkg.strip())

        install = ('yarn', 'global', 'install', pkg, *additional_dependencies)
